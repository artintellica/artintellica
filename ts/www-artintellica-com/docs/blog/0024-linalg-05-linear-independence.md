...
non-trivial combination (i.e., some $ c_i \neq 0 $) yields the zero vector, the
vectors are **linearly dependent**, meaning at least one vector is redundant.
...
- $ \mathbf{v}\_1 = [1, 2]
  $, $ \mathbf{v}\_2 = [2, 4] $ are linearly
  dependent ($ \mathbf{v}\_2 = 2
  \mathbf{v}\_1 $).
...
- The span of $ [1, 2] $ and $ [2, 4] $ as a line (dependent, since $ [2, 4] = 2
  \cdot [1, 2] $).
