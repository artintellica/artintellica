function wordBreak(str: string, wordDict: string[]): string[] | null {
  //strategy:
  ...
  const dp: (string[] | null)[] = new Array(str.length + 1).fill(null);
  ...
  for (let idx = 1; idx <= str.length; idx++) {
    ...
    for (let jdx = 0; jdx < idx; jdx++) {
      ...
      if (dp[jdx] && wordSet.has(str.substring(jdx, idx))) {
        dp[idx] = [...(dp[jdx] as string[]), str.substring(jdx, idx)];
        break; // no need to check further once we find a valid split
      }
    }
  }

  return dp[str.length] as string[] | null;
}
