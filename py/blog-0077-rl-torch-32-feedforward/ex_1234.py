...
# Weights and biases
W1: torch.Tensor = torch.randn(input_dim, hidden_dim, requires_grad=True)  # (2, 6)
b1: torch.Tensor = torch.zeros(hidden_dim, requires_grad=True)  # (6,)
W2: torch.Tensor = torch.randn(hidden_dim, output_dim, requires_grad=True)  # (6, 2)
b2: torch.Tensor = torch.zeros(output_dim, requires_grad=True)  # (2,)
# Forward, example for a batch X of shape (N, 2)


def forward(X: torch.Tensor) -> torch.Tensor:
    z1: torch.Tensor = X @ W1 + b1  # (N, 6)
    h: torch.Tensor = relu(z1)  # (N, 6)
...
plt.title("Two-Layer Neural Network Training Loss")
plt.grid(True)
plt.show()
