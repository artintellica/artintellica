...
with torch.no_grad():
    grid_x, grid_y = torch.meshgrid(
        torch.linspace(-5, 5, 100), torch.linspace(-5, 5, 100)
    )
    grid_points = torch.stack([grid_x.ravel(), grid_y.ravel()], dim=1)  # (10000, 2)
    grid_logits = grid_points @ W + b  # (10000, 3)
    grid_probs = softmax(grid_logits)  # (10000, 3)
    grid_classes = grid_probs.argmax(dim=1).reshape(grid_x.shape)  # (100, 100)
plt.contourf(
    grid_x,
    grid_y,
    grid_classes.numpy(),
